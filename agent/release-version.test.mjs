  ...
  for (const vector of vectors) await t.test(JSON.stringify(vector.tag), () => {
    ...
    for (const name of ['install.sh', 'install-linux.sh', 'install-windows.ps1']) {
      ...
      if (vector.pin && result.status === 0) {
        const expected = vector.tag ? `https://github.com/sbaliyun/esa-vps-monitor/releases/download/${encodeURIComponent(vector.tag)}`
          : 'https://github.com/sbaliyun/esa-vps-monitor/releases/latest/download';
