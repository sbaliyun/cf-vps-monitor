...
- **SSL 证书到期提醒**：Agent 读取 HTTPS 站点证书的到期时间，页面显示剩余天数，到期前每天提醒（见下文）。
...
ESA 边缘函数拿不到对方证书，证书由 Agent 检查：

- 默认自动选一个在线、且 Agent 版本支持证书检查的节点，每小时检查一次全部 HTTPS 网站；也可以在后台 **网站 → SSL 证书到期检查** 中指定节点或关闭。
- 网站列表显示「证书 N 天」：超过提醒天数为绿色，进入提醒期为黄色，3 天内或已过期为红色。
- 剩余天数不超过「提前提醒天数」（默认 14 天）时，每天通过「通知管理」里的渠道提醒一次；换证后自动停止。
- 需要新版 Agent（在 Actions 里运行 Agent Release 发布后，用后台安装命令升级至少一个节点）。旧版 Agent 不影响其他功能。
