...
const Game = (() => {
  ...
  function toTitle(first) {
    ...
    titleT = 0;
    // scenic title spot: the ruined-pagoda clearing, looking into the low sun
    const A0 = 0.95;
    player.respawn(new THREE.Vector3(7, 0, 5), A0 - 0.55);
    gcam.cinematic = (dt, gc) => {
      titleT += dt;
      const a = A0 + Math.sin(titleT * 0.05) * 0.3;
      const c = player.pos;
      const r = 3.6;
  ...
  function startGame() {
    ...
    setTimeout(() => {
      player.respawn(lastShrine.spawn, lastShrine.yaw);
