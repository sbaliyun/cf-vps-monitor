...
float wxNoise(vec3 x) { vec3 i = floor(x); vec3 f = fract(x); f = f * f * (3.0 - 2.0 * f);
  ...
  function beastGeo(s) {
    ...
    g.rotateY(-PI / 2); prep(g); uvProj(g, 2);
  ...
  function hallRoof(b, o) {
    ...
    if (o.type === 'xieshan') {
      ...
      if (o.chiwen !== false) {
        const cw = chiwenGeo(ridgeH * 2.6, ridgeW * 0.9);
  ...
  function buildKeeperShrine() {
    ...
    const r = hallRoof(b, { type: 'hip', ax, az, pitch: 0.95, ov, soffit: y0 + bh + 0.36, th: 0.16, ridgeW: 0.18, ridgeH: 0.2, curl: 0.06, tipS: 0.3, beasts: 0, ridgeCurve: 0.04, chiwenS: 0.55,
  ...
  function buildTempleHall({ width = 22, depth = 14, plaque = '观音禅院' } = {}) {
    ...
    const uxs = lin(-ux, ux, nbx >= 5 ? 3 : 2), uzs = lin(-uz, uz, 2), uTop = yJ + 1.75, uBase = yJ - 0.8;
    ...
    const ov2 = 2.2;
    const upper = hallRoof(b, { type: 'xieshan', ax: ux + ov2, az: uz + ov2, pitch: 0.66, ov: ov2, soffit: yB2 + hb2, th: 0.36, skirtIn: 1.0, og: 0.5,
      ridgeW: 0.42, ridgeH: 0.85, ridgeCurve: 0.25, curl: 0.32, beasts: 5, beastS: 1.0,
  ...
  function buildPagoda({ levels = 7, ruined = false } = {}) {
    ...
    for (let i = 0; i < kept; i++) {
      ...
      if (!isTop) {
        const faces2 = polyFaces(N, apN + 0.02, 0);
        void faces2;
      }
      if (isTop) {
        // 塔刹 finial
